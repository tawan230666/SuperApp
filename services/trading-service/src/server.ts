...
import {control,createOrder,getOrder,getPosition,getTrade,cancelOrder,closePosition,execute,reconcile,recover,status,list,heartbeat,monitor} from './paper.js';
...
app.get('/v1/positions/:id',async(req,res)=>res.json(await getPosition(res.locals.identity.id,id(req.params.id))));
app.get('/v1/trades/:id',async(req,res)=>res.json(await getTrade(res.locals.identity.id,id(req.params.id))));
...
app.get('/v1/reconciliation',async(_req,res)=>res.json(await reconcile(res.locals.identity.id)));
finish(app);
if(process.env.NODE_ENV!=='test'){
 await recover();await heartbeat();
 const timer=setInterval(()=>{heartbeat().then(()=>monitor()).catch(()=>{});},5000);timer.unref();
